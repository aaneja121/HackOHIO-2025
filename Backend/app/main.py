...
def login(payload: schemas.LoginRequest, session: Session = Depends(get_session)):
    ...
    if not user:
        ...
        session.add(user)
        session.commit()
    return schemas.LoginResponse(api_key="demo-key-123")  # align with .env

# ---------- Checklist (hard-coded demo) ----------
...
# ---------- Symptoms (very simple urgency heuristic) ----------
@app.post("/symptoms", response_model=schemas.SymptomOut, dependencies=[Depends(require_api_key)])
async def log_symptom(payload: schemas.SymptomIn, user_external_id: str = Form(...), session: Session = Depends(get_session)):
    user = session.exec(select(User).where(User.external_id == user_external_id)).first()
    if not user:
        raise HTTPException(400, "Unknown user_external_id")

    text = payload.text.lower()
    # dumb keyword urgency; replace with proper NLU later
    keywords = {"throbbing": 0.7, "dizzy": 0.6, "fever": 0.8, "pus": 0.9, "redness": 0.5, "pain": 0.5}
    urgency = max([w for k, w in keywords.items() if k in text] + [0.2])  # default low
